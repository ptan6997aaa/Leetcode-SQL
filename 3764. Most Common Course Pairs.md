...
def topLearnerCourseTransitions(course_completions: pd.DataFrame) -> pd.DataFrame:
    # user completed at least 5 courses with average rating of 4 or higher 
    top_performing_user = course_completions.groupby('user_id').agg(
        cnt=('course_id' ,'count'),
        avg=('course_rating', 'mean')       
    ).reset_index()
    top_performing_user = top_performing_user.loc[
        (top_performing_user['cnt'] >= 5) & 
        (top_performing_user['avg'] >= 4)
        ]
    # sort course for each top performer 
    filter_top_performer = course_completions.merge(top_performing_user, how='inner', on='user_id')
    filter_top_performer = filter_top_performer[['user_id', 'course_name', 'completion_date']]
    filter_top_performer['completion_date'] = pd.to_datetime(filter_top_performer['completion_date'])
    filter_top_performer = filter_top_performer.sort_values(['user_id', 'completion_date'], ascending=[True, True]) 
    # shift course 
    filter_top_performer['next_course'] = filter_top_performer.groupby('user_id')['course_name'].shift(-1) 
    filter_top_performer = filter_top_performer[['course_name', 'next_course']]
    # output 
    output = filter_top_performer.groupby(['course_name', 'next_course']).agg(
        transition_count=('next_course', 'count')       
    ).reset_index() 
    output = output.rename(columns={'course_name': 'first_course', 'next_course': 'second_course'})
    output['dummy_first_course'] = output['first_course'].str.lower()
    output['dummy_second_course'] = output['second_course'].str.lower()
    output = output.sort_values(['transition_count', 'dummy_first_course', 'dummy_second_course'], ascending=[False, True, True])
    output = output.drop(columns=['dummy_first_course', 'dummy_second_course'])  
    return output
```
